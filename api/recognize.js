...
const SCHEMA = {
  ...
  properties: {
    productName: {
      type: 'string',
      description: '出品タイトルに使える簡潔な商品名（日本語可）。iPhoneなら機種名、トレカ単品ならカード名、BOXならBOX名、フィギュアなら作品＋キャラ名、本・漫画なら検索しやすいタイトル＋レーベル/文庫名/出版社/シリーズ名（例: 超かぐや姫! ファミ通文庫。巻数が明確に見える場合は「タイトル ◯巻」）など。【重要】画像に見えるブランド名は必ず先頭に含めること（例: 無印良品 さっぱり乳液 / excel 化粧下地 / 資生堂 TSUBAKI シャンプー）。「詳細不明」「プラスチックケース入り商品」「説明不明」など汎用的な記述は絶対に使わないこと。'
    },
    category: {
      type: 'string',
    ...
    brand: {
      type: 'string',
      description: 'ブランド/メーカー/IP。必ず画像に見える文字からブランドを特定すること（例: Apple, ポケモン, BANDAI, 無印良品, excel, 資生堂, DHC, KOSE, SK-II, カネボウ）。不明なら空文字。'
    },
    series: {
      type: 'string',
    ...
    storage: {
      type: 'string',
      description: 'iPhoneならストレージ容量（例: 256GB）、game_softwareならプラットフォーム名（Nintendo Switch / PS4 / PS5 / Xbox Series X / 3DS 等）、game_consoleならモデル名・容量（例: Nintendo Switch OLED / PlayStation 5 Digital Edition / 512GB）。それ以外は空文字。'
    },
    color: {
      type: 'string',
      description: 'iPhone/game_consoleの本体色（例: Graphite / ホワイト / ブラック）。読み取れなければ空文字。'
    },
    condition: {
      type: 'string',
